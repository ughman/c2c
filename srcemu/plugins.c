...
int LoadPlugins() {
	...
	if (UsingIso()) {
		...
		if (LoadCDRplugin("psx-cdr.dll") == -1) return -1;
	}

	sprintf(Plugin, "%s/%s", Config.PluginsDir, Config.Gpu);
	if (LoadGPUplugin("psx-gpu.dll") == -1) return -1;

	sprintf(Plugin, "%s/%s", Config.PluginsDir, Config.Spu);
	if (LoadSPUplugin("psx-spu.dll") == -1) return -1;

	sprintf(Plugin, "%s/%s", Config.PluginsDir, Config.Pad1);
	if (LoadPAD1plugin("psx-pad1.dll") == -1) return -1;

	sprintf(Plugin, "%s/%s", Config.PluginsDir, Config.Pad2);
	if (LoadPAD2plugin("psx-pad2.dll") == -1) return -1;
