#ifdef ZZ_INCLUDE_CODE
ZZ_42FA8:
	T2 = T9 << 12;
	if ((int32_t)T2 < 0)
	{
		T9 <<= 13;
		ZZ_CLOCKCYCLES(3,0x800430E8);
		goto ZZ_42FA8_140;
	}
	T9 <<= 13;
	if ((int32_t)T9 < 0)
	{
		A3 = 4896;
		ZZ_CLOCKCYCLES(5,0x800430E0);
		goto ZZ_42FA8_138;
	}
	A3 = 4896;
	T2 = A0 - T0;
	if ((int32_t)T2 >= 0)
	{
		A2 = T8 - T3;
		ZZ_CLOCKCYCLES(8,0x80042FCC);
		goto ZZ_42FA8_24;
	}
	A2 = T8 - T3;
	T2 = -T2;
	ZZ_CLOCKCYCLES(9,0x80042FCC);
ZZ_42FA8_24:
	A3 = A3 - T2;
	if ((int32_t)A3 < 0)
	{
		T2 = (int32_t)A2 >> 16;
		ZZ_CLOCKCYCLES(3,0x80043008);
		goto ZZ_42FA8_60;
	}
	T2 = (int32_t)A2 >> 16;
	if ((int32_t)T2 >= 0)
	{
		A2 <<= 16;
		ZZ_CLOCKCYCLES(5,0x80042FE4);
		goto ZZ_42FA8_3C;
	}
	A2 <<= 16;
	T2 = -T2;
	ZZ_CLOCKCYCLES(6,0x80042FE4);
ZZ_42FA8_3C:
	A3 = A3 - T2;
	if ((int32_t)A3 < 0)
	{
		T2 = (int32_t)A2 >> 16;
		ZZ_CLOCKCYCLES(3,0x80043008);
		goto ZZ_42FA8_60;
	}
	T2 = (int32_t)A2 >> 16;
	if ((int32_t)T2 >= 0)
	{
		A3 = EMU_CheckedAdd(A3,-800);
		ZZ_CLOCKCYCLES(5,0x80042FFC);
		goto ZZ_42FA8_54;
	}
	A3 = EMU_CheckedAdd(A3,-800);
	T2 = -T2;
	ZZ_CLOCKCYCLES(6,0x80042FFC);
ZZ_42FA8_54:
	A3 = A3 - T2;
	if ((int32_t)A3 >= 0)
	{
		ZZ_CLOCKCYCLES(3,0x8004300C);
		goto ZZ_42FA8_64;
	}
	ZZ_CLOCKCYCLES(3,0x80043008);
ZZ_42FA8_60:
	A3 = 0;
	ZZ_CLOCKCYCLES(1,0x8004300C);
ZZ_42FA8_64:
	AT = 4896;
	T2 = A0 - S6;
	if ((int32_t)T2 >= 0)
	{
		A2 = T8 - S2;
		ZZ_CLOCKCYCLES(4,0x80043020);
		goto ZZ_42FA8_78;
	}
	A2 = T8 - S2;
	T2 = -T2;
	ZZ_CLOCKCYCLES(5,0x80043020);
ZZ_42FA8_78:
	AT = AT - T2;
	if ((int32_t)AT < 0)
	{
		T2 = (int32_t)A2 >> 16;
		ZZ_CLOCKCYCLES(3,0x8004305C);
		goto ZZ_42FA8_B4;
	}
	T2 = (int32_t)A2 >> 16;
	if ((int32_t)T2 >= 0)
	{
		A2 <<= 16;
		ZZ_CLOCKCYCLES(5,0x80043038);
		goto ZZ_42FA8_90;
	}
	A2 <<= 16;
	T2 = -T2;
	ZZ_CLOCKCYCLES(6,0x80043038);
ZZ_42FA8_90:
	AT = AT - T2;
	if ((int32_t)AT < 0)
	{
		T2 = (int32_t)A2 >> 16;
		ZZ_CLOCKCYCLES(3,0x8004305C);
		goto ZZ_42FA8_B4;
	}
	T2 = (int32_t)A2 >> 16;
	if ((int32_t)T2 >= 0)
	{
		AT = EMU_CheckedAdd(AT,-800);
		ZZ_CLOCKCYCLES(5,0x80043050);
		goto ZZ_42FA8_A8;
	}
	AT = EMU_CheckedAdd(AT,-800);
	T2 = -T2;
	ZZ_CLOCKCYCLES(6,0x80043050);
ZZ_42FA8_A8:
	AT = AT - T2;
	if ((int32_t)AT >= 0)
	{
		ZZ_CLOCKCYCLES(3,0x80043060);
		goto ZZ_42FA8_B8;
	}
	ZZ_CLOCKCYCLES(3,0x8004305C);
ZZ_42FA8_B4:
	AT = 0;
	ZZ_CLOCKCYCLES(1,0x80043060);
ZZ_42FA8_B8:
	T2 = A3 + AT;
	if (!T2)
	{
		A2 = 4096;
		ZZ_CLOCKCYCLES(3,0x800430C0);
		goto ZZ_42FA8_118;
	}
	A2 = 4096;
	T2 = A2 - T2;
	if ((int32_t)T2 <= 0)
	{
		T8 = EMU_ReadU32(V1 + 496); //+ 0x1F0
		ZZ_CLOCKCYCLES(6,0x800430AC);
		goto ZZ_42FA8_104;
	}
	T8 = EMU_ReadU32(V1 + 496); //+ 0x1F0
	T9 = EMU_ReadU32(V1 + 500); //+ 0x1F4
	A0 = EMU_ReadU32(V1 + 504); //+ 0x1F8
	GTE_SetControlRegister(21,T8);
	GTE_SetControlRegister(22,T9);
	GTE_SetControlRegister(23,A0);
	GTE_SetDataRegister(6,EMU_ReadU32(V1 + 144)); //+ 0x90
	GTE_SetDataRegister(8,T2);
	GTE_Execute(7864336);
	GTE_GetDataRegister(9);
	ZZ_CLOCKCYCLES(19,0x800430CC);
	goto ZZ_42FA8_124;
ZZ_42FA8_104:
	GTE_SetDataRegister(9,EMU_ReadU32(V1 + 520)); //+ 0x208
	GTE_SetDataRegister(10,EMU_ReadU32(V1 + 524)); //+ 0x20C
	GTE_SetDataRegister(11,EMU_ReadU32(V1 + 528)); //+ 0x210
	ZZ_CLOCKCYCLES(5,0x800430CC);
	goto ZZ_42FA8_124;
ZZ_42FA8_118:
	GTE_SetDataRegister(9,EMU_ReadU32(V1 + 496)); //+ 0x1F0
	GTE_SetDataRegister(10,EMU_ReadU32(V1 + 500)); //+ 0x1F4
	GTE_SetDataRegister(11,EMU_ReadU32(V1 + 504)); //+ 0x1F8
	ZZ_CLOCKCYCLES(3,0x800430CC);
ZZ_42FA8_124:
	GTE_SetDataRegister(8,R0);
	GTE_SetDataRegister(6,EMU_ReadU32(V1 + 200)); //+ 0xC8
	GTE_Execute(6815785);
	ZZ_CLOCKCYCLES(5,0x800430E0);
ZZ_42FA8_138:
	ZZ_JUMPREGISTER_BEGIN(RA);
	ZZ_CLOCKCYCLES_JR(2);
	ZZ_JUMPREGISTER(0x80043154,ZZ_42FA8_1AC);
	ZZ_JUMPREGISTER(0x800431DC,ZZ_42FA8_234);
	ZZ_JUMPREGISTER(0x8004316C,ZZ_42FA8_1C4);
	ZZ_JUMPREGISTER(0x800431F4,ZZ_42FA8_24C);
	ZZ_JUMPREGISTER(0x80043184,ZZ_42FA8_1DC);
	ZZ_JUMPREGISTER(0x8004320C,ZZ_42FA8_264);
	ZZ_JUMPREGISTER(0x80043224,ZZ_42FA8_27C);
	ZZ_JUMPREGISTER_END();
ZZ_42FA8_140:
	T8 = EMU_ReadU32(V1 + 508); //+ 0x1FC
	T9 = EMU_ReadU32(V1 + 512); //+ 0x200
	A0 = EMU_ReadU32(V1 + 516); //+ 0x204
	GTE_SetControlRegister(21,T8);
	GTE_SetControlRegister(22,T9);
	GTE_SetControlRegister(23,A0);
	GTE_SetDataRegister(8,EMU_ReadU32(V1 + 532)); //+ 0x214
	GTE_SetDataRegister(6,EMU_ReadU32(V1 + 200)); //+ 0xC8
	GTE_Execute(7864336);
	ZZ_JUMPREGISTER_BEGIN(RA);
	ZZ_CLOCKCYCLES_JR(13);
	ZZ_JUMPREGISTER(0x80043154,ZZ_42FA8_1AC);
	ZZ_JUMPREGISTER(0x800431DC,ZZ_42FA8_234);
	ZZ_JUMPREGISTER(0x8004316C,ZZ_42FA8_1C4);
	ZZ_JUMPREGISTER(0x800431F4,ZZ_42FA8_24C);
	ZZ_JUMPREGISTER(0x80043184,ZZ_42FA8_1DC);
	ZZ_JUMPREGISTER(0x8004320C,ZZ_42FA8_264);
	ZZ_JUMPREGISTER(0x80043224,ZZ_42FA8_27C);
	ZZ_JUMPREGISTER_END();
ZZ_42FA8_174:
	EMU_Write32(V1 + 176,AT); //+ 0xB0
	EMU_Write32(V1 + 180,A3); //+ 0xB4
	EMU_Write32(V1 + 184,T0); //+ 0xB8
	EMU_Write32(V1 + 120,T3); //+ 0x78
	EMU_Write32(V1 + 392,RA); //+ 0x188
	T3 = EMU_ReadU32(V1 + 124); //+ 0x7C
	S2 = EMU_ReadU32(V1 + 136); //+ 0x88
	T0 = EMU_ReadU32(V1 + 128); //+ 0x80
	S6 = EMU_ReadU32(V1 + 140); //+ 0x8C
	T8 = GTE_GetDataRegister(0);
	A0 = GTE_GetDataRegister(1);
	T9 = EMU_ReadU32(V1 + 176); //+ 0xB0
	RA = 0x80043154; //ZZ_42FA8_1AC
	EMU_Write32(V1 + 200,T5); //+ 0xC8
	ZZ_CLOCKCYCLES(14,0x80042FA8);
	goto ZZ_42FA8;
ZZ_42FA8_1AC:
	T8 = GTE_GetDataRegister(2);
	A0 = GTE_GetDataRegister(3);
	T5 = GTE_GetDataRegister(22);
	T9 = EMU_ReadU32(V1 + 180); //+ 0xB4
	RA = 0x8004316C; //ZZ_42FA8_1C4
	EMU_Write32(V1 + 200,T6); //+ 0xC8
	ZZ_CLOCKCYCLES(6,0x80042FA8);
	goto ZZ_42FA8;
ZZ_42FA8_1C4:
	T8 = GTE_GetDataRegister(4);
	A0 = GTE_GetDataRegister(5);
	T6 = GTE_GetDataRegister(22);
	T9 = EMU_ReadU32(V1 + 184); //+ 0xB8
	RA = 0x80043184; //ZZ_42FA8_1DC
	EMU_Write32(V1 + 200,T7); //+ 0xC8
	ZZ_CLOCKCYCLES(6,0x80042FA8);
	goto ZZ_42FA8;
ZZ_42FA8_1DC:
	T7 = GTE_GetDataRegister(22);
	T3 = EMU_ReadU32(V1 + 120); //+ 0x78
	S6 = EMU_ReadU32(V1 + 472); //+ 0x1D8
	T2 = EMU_ReadU32(V1 + 476); //+ 0x1DC
	RA = EMU_ReadU32(V1 + 392); //+ 0x188
	ZZ_CLOCKCYCLES(7,0x800427E0);
	goto ZZ_420F4_6EC;
ZZ_42FA8_1F8:
	EMU_Write32(V1 + 176,AT); //+ 0xB0
	EMU_Write32(V1 + 180,A3); //+ 0xB4
	EMU_Write32(V1 + 184,T0); //+ 0xB8
	EMU_Write32(V1 + 188,A0); //+ 0xBC
	EMU_Write32(V1 + 120,T3); //+ 0x78
	EMU_Write32(V1 + 392,RA); //+ 0x188
	T3 = EMU_ReadU32(V1 + 124); //+ 0x7C
	S2 = EMU_ReadU32(V1 + 136); //+ 0x88
	T0 = EMU_ReadU32(V1 + 128); //+ 0x80
	S6 = EMU_ReadU32(V1 + 140); //+ 0x8C
	T8 = EMU_ReadU32(V1 + 436); //+ 0x1B4
	A0 = EMU_ReadU32(V1 + 440); //+ 0x1B8
	T9 = EMU_ReadU32(V1 + 176); //+ 0xB0
	RA = 0x800431DC; //ZZ_42FA8_234
	EMU_Write32(V1 + 200,T5); //+ 0xC8
	ZZ_CLOCKCYCLES(15,0x80042FA8);
	goto ZZ_42FA8;
ZZ_42FA8_234:
	T8 = GTE_GetDataRegister(2);
	A0 = GTE_GetDataRegister(3);
	T5 = GTE_GetDataRegister(22);
	T9 = EMU_ReadU32(V1 + 180); //+ 0xB4
	RA = 0x800431F4; //ZZ_42FA8_24C
	EMU_Write32(V1 + 200,T6); //+ 0xC8
	ZZ_CLOCKCYCLES(6,0x80042FA8);
	goto ZZ_42FA8;
ZZ_42FA8_24C:
	T8 = GTE_GetDataRegister(4);
	A0 = GTE_GetDataRegister(5);
	T6 = GTE_GetDataRegister(22);
	T9 = EMU_ReadU32(V1 + 184); //+ 0xB8
	RA = 0x8004320C; //ZZ_42FA8_264
	EMU_Write32(V1 + 200,T7); //+ 0xC8
	ZZ_CLOCKCYCLES(6,0x80042FA8);
	goto ZZ_42FA8;
ZZ_42FA8_264:
	T8 = GTE_GetDataRegister(0);
	A0 = GTE_GetDataRegister(1);
	T7 = GTE_GetDataRegister(22);
	T9 = EMU_ReadU32(V1 + 188); //+ 0xBC
	RA = 0x80043224; //ZZ_42FA8_27C
	EMU_Write32(V1 + 200,A1); //+ 0xC8
	ZZ_CLOCKCYCLES(6,0x80042FA8);
	goto ZZ_42FA8;
ZZ_42FA8_27C:
	A1 = GTE_GetDataRegister(22);
	T3 = EMU_ReadU32(V1 + 120); //+ 0x78
	S6 = EMU_ReadU32(V1 + 472); //+ 0x1D8
	T2 = EMU_ReadU32(V1 + 476); //+ 0x1DC
	RA = EMU_ReadU32(V1 + 392); //+ 0x188
	ZZ_CLOCKCYCLES(7,0x80042AB0);
	goto ZZ_420F4_9BC;
#endif
ZZ_MARK_TARGET(0x80042FA8,ZZ_42FA8);
ZZ_MARK(0x80042FAC);
ZZ_MARK(0x80042FB0);
ZZ_MARK(0x80042FB4);
ZZ_MARK(0x80042FB8);
ZZ_MARK(0x80042FBC);
ZZ_MARK(0x80042FC0);
ZZ_MARK(0x80042FC4);
ZZ_MARK(0x80042FC8);
ZZ_MARK_TARGET(0x80042FCC,ZZ_42FA8_24);
ZZ_MARK(0x80042FD0);
ZZ_MARK(0x80042FD4);
ZZ_MARK(0x80042FD8);
ZZ_MARK(0x80042FDC);
ZZ_MARK(0x80042FE0);
ZZ_MARK_TARGET(0x80042FE4,ZZ_42FA8_3C);
ZZ_MARK(0x80042FE8);
ZZ_MARK(0x80042FEC);
ZZ_MARK(0x80042FF0);
ZZ_MARK(0x80042FF4);
ZZ_MARK(0x80042FF8);
ZZ_MARK_TARGET(0x80042FFC,ZZ_42FA8_54);
ZZ_MARK(0x80043000);
ZZ_MARK(0x80043004);
ZZ_MARK_TARGET(0x80043008,ZZ_42FA8_60);
ZZ_MARK_TARGET(0x8004300C,ZZ_42FA8_64);
ZZ_MARK(0x80043010);
ZZ_MARK(0x80043014);
ZZ_MARK(0x80043018);
ZZ_MARK(0x8004301C);
ZZ_MARK_TARGET(0x80043020,ZZ_42FA8_78);
ZZ_MARK(0x80043024);
ZZ_MARK(0x80043028);
ZZ_MARK(0x8004302C);
ZZ_MARK(0x80043030);
ZZ_MARK(0x80043034);
ZZ_MARK_TARGET(0x80043038,ZZ_42FA8_90);
ZZ_MARK(0x8004303C);
ZZ_MARK(0x80043040);
ZZ_MARK(0x80043044);
ZZ_MARK(0x80043048);
ZZ_MARK(0x8004304C);
ZZ_MARK_TARGET(0x80043050,ZZ_42FA8_A8);
ZZ_MARK(0x80043054);
ZZ_MARK(0x80043058);
ZZ_MARK_TARGET(0x8004305C,ZZ_42FA8_B4);
ZZ_MARK_TARGET(0x80043060,ZZ_42FA8_B8);
ZZ_MARK(0x80043064);
ZZ_MARK(0x80043068);
ZZ_MARK(0x8004306C);
ZZ_MARK(0x80043070);
ZZ_MARK(0x80043074);
ZZ_MARK(0x80043078);
ZZ_MARK(0x8004307C);
ZZ_MARK(0x80043080);
ZZ_MARK(0x80043084);
ZZ_MARK(0x80043088);
ZZ_MARK(0x8004308C);
ZZ_MARK(0x80043090);
ZZ_MARK(0x80043094);
ZZ_MARK(0x80043098);
ZZ_MARK(0x8004309C);
ZZ_MARK(0x800430A0);
ZZ_MARK(0x800430A4);
ZZ_MARK(0x800430A8);
ZZ_MARK_TARGET(0x800430AC,ZZ_42FA8_104);
ZZ_MARK(0x800430B0);
ZZ_MARK(0x800430B4);
ZZ_MARK(0x800430B8);
ZZ_MARK(0x800430BC);
ZZ_MARK_TARGET(0x800430C0,ZZ_42FA8_118);
ZZ_MARK(0x800430C4);
ZZ_MARK(0x800430C8);
ZZ_MARK_TARGET(0x800430CC,ZZ_42FA8_124);
ZZ_MARK(0x800430D0);
ZZ_MARK(0x800430D4);
ZZ_MARK(0x800430D8);
ZZ_MARK(0x800430DC);
ZZ_MARK_TARGET(0x800430E0,ZZ_42FA8_138);
ZZ_MARK(0x800430E4);
ZZ_MARK_TARGET(0x800430E8,ZZ_42FA8_140);
ZZ_MARK(0x800430EC);
ZZ_MARK(0x800430F0);
ZZ_MARK(0x800430F4);
ZZ_MARK(0x800430F8);
ZZ_MARK(0x800430FC);
ZZ_MARK(0x80043100);
ZZ_MARK(0x80043104);
ZZ_MARK(0x80043108);
ZZ_MARK(0x8004310C);
ZZ_MARK(0x80043110);
ZZ_MARK(0x80043114);
ZZ_MARK(0x80043118);
ZZ_MARK_TARGET(0x8004311C,ZZ_42FA8_174);
ZZ_MARK(0x80043120);
ZZ_MARK(0x80043124);
ZZ_MARK(0x80043128);
ZZ_MARK(0x8004312C);
ZZ_MARK(0x80043130);
ZZ_MARK(0x80043134);
ZZ_MARK(0x80043138);
ZZ_MARK(0x8004313C);
ZZ_MARK(0x80043140);
ZZ_MARK(0x80043144);
ZZ_MARK(0x80043148);
ZZ_MARK(0x8004314C);
ZZ_MARK(0x80043150);
ZZ_MARK_TARGET(0x80043154,ZZ_42FA8_1AC);
ZZ_MARK(0x80043158);
ZZ_MARK(0x8004315C);
ZZ_MARK(0x80043160);
ZZ_MARK(0x80043164);
ZZ_MARK(0x80043168);
ZZ_MARK_TARGET(0x8004316C,ZZ_42FA8_1C4);
ZZ_MARK(0x80043170);
ZZ_MARK(0x80043174);
ZZ_MARK(0x80043178);
ZZ_MARK(0x8004317C);
ZZ_MARK(0x80043180);
ZZ_MARK_TARGET(0x80043184,ZZ_42FA8_1DC);
ZZ_MARK(0x80043188);
ZZ_MARK(0x8004318C);
ZZ_MARK(0x80043190);
ZZ_MARK(0x80043194);
ZZ_MARK(0x80043198);
ZZ_MARK(0x8004319C);
ZZ_MARK_TARGET(0x800431A0,ZZ_42FA8_1F8);
ZZ_MARK(0x800431A4);
ZZ_MARK(0x800431A8);
ZZ_MARK(0x800431AC);
ZZ_MARK(0x800431B0);
ZZ_MARK(0x800431B4);
ZZ_MARK(0x800431B8);
ZZ_MARK(0x800431BC);
ZZ_MARK(0x800431C0);
ZZ_MARK(0x800431C4);
ZZ_MARK(0x800431C8);
ZZ_MARK(0x800431CC);
ZZ_MARK(0x800431D0);
ZZ_MARK(0x800431D4);
ZZ_MARK(0x800431D8);
ZZ_MARK_TARGET(0x800431DC,ZZ_42FA8_234);
ZZ_MARK(0x800431E0);
ZZ_MARK(0x800431E4);
ZZ_MARK(0x800431E8);
ZZ_MARK(0x800431EC);
ZZ_MARK(0x800431F0);
ZZ_MARK_TARGET(0x800431F4,ZZ_42FA8_24C);
ZZ_MARK(0x800431F8);
ZZ_MARK(0x800431FC);
ZZ_MARK(0x80043200);
ZZ_MARK(0x80043204);
ZZ_MARK(0x80043208);
ZZ_MARK_TARGET(0x8004320C,ZZ_42FA8_264);
ZZ_MARK(0x80043210);
ZZ_MARK(0x80043214);
ZZ_MARK(0x80043218);
ZZ_MARK(0x8004321C);
ZZ_MARK(0x80043220);
ZZ_MARK_TARGET(0x80043224,ZZ_42FA8_27C);
ZZ_MARK(0x80043228);
ZZ_MARK(0x8004322C);
ZZ_MARK(0x80043230);
ZZ_MARK(0x80043234);
ZZ_MARK(0x80043238);
ZZ_MARK(0x8004323C);
