#ifdef ZZ_INCLUDE_CODE
ZZ_18690:
	T2 = A0;
	V1 = EMU_ReadU32(A3 + 16); //+ 0x10
	A0 = EMU_ReadU16(A3);
	T3 = EMU_ReadU32(SP + 16); //+ 0x10
	V1 <<= 16;
	A2 = A0 | V1;
	GTE_SetDataRegister(0,A2);
	T4 = R0;
	GTE_SetDataRegister(1,T4);
	V0 = EMU_ReadU16(A3 + 12); //+ 0xC
	A2 = V0 | V1;
	GTE_SetDataRegister(2,A2);
	GTE_SetDataRegister(3,T4);
	V0 = EMU_ReadU32(A3 + 4); //+ 0x4
	V0 <<= 16;
	A2 = A0 | V0;
	GTE_SetDataRegister(4,A2);
	GTE_SetDataRegister(5,T4);
	GTE_Execute(2621488);
	A2 = GTE_GetControlRegister(31);
	if ((int32_t)A2 < 0)
	{
		ZZ_CLOCKCYCLES(25,0x800187F4);
		goto ZZ_18690_164;
	}
	V1 = 0x80060000;
	V1 = EMU_ReadU32(V1 + 14064); //+ 0x36F0
	A0 = EMU_ReadU32(V1 + 8); //+ 0x8
	V0 = A0 + 40;
	EMU_Write32(V1 + 8,V0); //+ 0x8
	V0 = EMU_ReadU32(T2);
	T1 = A0;
	EMU_Write32(A0 + 4,V0); //+ 0x4
	T0 = GTE_GetDataRegister(17);
	A2 = GTE_GetDataRegister(18);
	EMU_Write32(T1 + 24,GTE_GetDataRegister(14)); //+ 0x18
	T0 += A2;
	A2 = GTE_GetDataRegister(19);
	EMU_Write32(T1 + 16,GTE_GetDataRegister(13)); //+ 0x10
	V0 = 0x80060000;
	V0 = EMU_ReadU32(V0 + 2844); //+ 0xB1C
	A0 = A1 + 2048;
	V1 = V0 >> 31;
	V0 += V1;
	V0 = (int32_t)V0 >> 1;
	A0 = A0 - V0;
	V0 = T0 + A2;
	V0 >>= 5;
	T0 = A0 - V0;
	V0 = (int32_t)T0 < 2047;
	if (!V0)
	{
		ZZ_CLOCKCYCLES(54,0x8001877C);
		goto ZZ_18690_EC;
	}
	if ((int32_t)T0 < 0)
	{
		A1 = T3;
		ZZ_CLOCKCYCLES(56,0x80018780);
		goto ZZ_18690_F0;
	}
	A1 = T3;
	V0 = T0 << 2;
	A1 += V0;
	ZZ_CLOCKCYCLES(59,0x80018780);
	goto ZZ_18690_F0;
ZZ_18690_EC:
	A1 = T3 + 8188;
	ZZ_CLOCKCYCLES(1,0x80018780);
ZZ_18690_F0:
	V1 = EMU_ReadU32(A1);
	A0 = 0xFF0000;
	A0 |= 0xFFFF;
	V0 = 0x9000000;
	A0 &= T1;
	V1 |= V0;
	EMU_Write32(T1,V1);
	EMU_Write32(A1,A0);
	V0 = EMU_ReadU32(A3 + 4); //+ 0x4
	V1 = EMU_ReadU16(A3 + 12); //+ 0xC
	V0 <<= 16;
	A2 = V1 | V0;
	EMU_Write32(T1 + 8,GTE_GetDataRegister(12)); //+ 0x8
	GTE_SetDataRegister(0,A2);
	T4 = R0;
	GTE_SetDataRegister(1,T4);
	GTE_Execute(1572865);
	V0 = EMU_ReadU32(T2 + 4); //+ 0x4
	EMU_Write32(T1 + 12,V0); //+ 0xC
	V0 = EMU_ReadU32(T2 + 8); //+ 0x8
	EMU_Write32(T1 + 20,V0); //+ 0x14
	V0 = EMU_ReadU32(T2 + 12); //+ 0xC
	EMU_Write16(T1 + 28,V0); //+ 0x1C
	V0 >>= 16;
	EMU_Write16(T1 + 36,V0); //+ 0x24
	EMU_Write32(T1 + 32,GTE_GetDataRegister(14)); //+ 0x20
	ZZ_CLOCKCYCLES(29,0x800187F4);
ZZ_18690_164:
	ZZ_JUMPREGISTER_BEGIN(RA);
	ZZ_CLOCKCYCLES_JR(2);
	ZZ_JUMPREGISTER(0x8001BE84,ZZ_1B974_510);
	ZZ_JUMPREGISTER_END();
#endif
ZZ_MARK_TARGET(0x80018690,ZZ_18690);
ZZ_MARK(0x80018694);
ZZ_MARK(0x80018698);
ZZ_MARK(0x8001869C);
ZZ_MARK(0x800186A0);
ZZ_MARK(0x800186A4);
ZZ_MARK(0x800186A8);
ZZ_MARK(0x800186AC);
ZZ_MARK(0x800186B0);
ZZ_MARK(0x800186B4);
ZZ_MARK(0x800186B8);
ZZ_MARK(0x800186BC);
ZZ_MARK(0x800186C0);
ZZ_MARK(0x800186C4);
ZZ_MARK(0x800186C8);
ZZ_MARK(0x800186CC);
ZZ_MARK(0x800186D0);
ZZ_MARK(0x800186D4);
ZZ_MARK(0x800186D8);
ZZ_MARK(0x800186DC);
ZZ_MARK(0x800186E0);
ZZ_MARK(0x800186E4);
ZZ_MARK(0x800186E8);
ZZ_MARK(0x800186EC);
ZZ_MARK(0x800186F0);
ZZ_MARK(0x800186F4);
ZZ_MARK(0x800186F8);
ZZ_MARK(0x800186FC);
ZZ_MARK(0x80018700);
ZZ_MARK(0x80018704);
ZZ_MARK(0x80018708);
ZZ_MARK(0x8001870C);
ZZ_MARK(0x80018710);
ZZ_MARK(0x80018714);
ZZ_MARK(0x80018718);
ZZ_MARK(0x8001871C);
ZZ_MARK(0x80018720);
ZZ_MARK(0x80018724);
ZZ_MARK(0x80018728);
ZZ_MARK(0x8001872C);
ZZ_MARK(0x80018730);
ZZ_MARK(0x80018734);
ZZ_MARK(0x80018738);
ZZ_MARK(0x8001873C);
ZZ_MARK(0x80018740);
ZZ_MARK(0x80018744);
ZZ_MARK(0x80018748);
ZZ_MARK(0x8001874C);
ZZ_MARK(0x80018750);
ZZ_MARK(0x80018754);
ZZ_MARK(0x80018758);
ZZ_MARK(0x8001875C);
ZZ_MARK(0x80018760);
ZZ_MARK(0x80018764);
ZZ_MARK(0x80018768);
ZZ_MARK(0x8001876C);
ZZ_MARK(0x80018770);
ZZ_MARK(0x80018774);
ZZ_MARK(0x80018778);
ZZ_MARK_TARGET(0x8001877C,ZZ_18690_EC);
ZZ_MARK_TARGET(0x80018780,ZZ_18690_F0);
ZZ_MARK(0x80018784);
ZZ_MARK(0x80018788);
ZZ_MARK(0x8001878C);
ZZ_MARK(0x80018790);
ZZ_MARK(0x80018794);
ZZ_MARK(0x80018798);
ZZ_MARK(0x8001879C);
ZZ_MARK(0x800187A0);
ZZ_MARK(0x800187A4);
ZZ_MARK(0x800187A8);
ZZ_MARK(0x800187AC);
ZZ_MARK(0x800187B0);
ZZ_MARK(0x800187B4);
ZZ_MARK(0x800187B8);
ZZ_MARK(0x800187BC);
ZZ_MARK(0x800187C0);
ZZ_MARK(0x800187C4);
ZZ_MARK(0x800187C8);
ZZ_MARK(0x800187CC);
ZZ_MARK(0x800187D0);
ZZ_MARK(0x800187D4);
ZZ_MARK(0x800187D8);
ZZ_MARK(0x800187DC);
ZZ_MARK(0x800187E0);
ZZ_MARK(0x800187E4);
ZZ_MARK(0x800187E8);
ZZ_MARK(0x800187EC);
ZZ_MARK(0x800187F0);
ZZ_MARK_TARGET(0x800187F4,ZZ_18690_164);
ZZ_MARK(0x800187F8);
